...
// `priseSp` est du contexte d'URL (l'écran `regle` le réclame dans la table des
// routes), donc l'espèce du test précédent survit dans le hash et le store la
// relit à l'hydratation. Sans ce ménage, « sans aucune espèce » hériterait du
// brochet posé deux tests plus haut, et passerait pour la mauvaise raison.
beforeEach(() => {
  localStorage.clear();
...
describe("Règle — elle ne mesure jamais une espèce que le pêcheur n'a pas demandée", () => {
  ...
  it("pendant le parcours « Ma prise », elle suit l'espèce du parcours", () => {
    ...
    // `priseSp` survit à `nav("regle")` parce que l'écran `regle` le réclame
    // dans la table des routes (nav-conventions.ts) — c'est ce qui faisait tenir
    // ce chemin-là pendant que celui de la fiche était cassé. Ce test le
    // verrouille.
    render(
      <StoreProvider>
        <Monte
