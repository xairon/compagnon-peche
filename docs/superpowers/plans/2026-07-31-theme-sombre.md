...
- **L'empreinte ne mesure que les couleurs APPLIQUÉES, pas les définitions de jetons** (amendement décidé pendant la Tâche 4). Les déclarations dont la propriété commence par `--` sont exclues du calcul. Sans ça, *déclarer* un jeton ferait bouger le fixture alors qu'aucun pixel ne change, et le conflit se reposerait à chaque tâche qui en ajoute (4, 5, 9). Changer la **valeur** d'un jeton reste détecté, puisque tous ses consommateurs bougent — la garantie n'est pas affaiblie, elle est rendue exacte.
